...
#+begin_src emacs-lisp :noweb-ref startup-time
  ...
  (use-package emacs
    :hook (emacs-startup . enfer/show-startup-time))

#+end_src
*** Install some popular packages
#+begin_src emacs-lisp :noweb-ref popular-packages
;; popular packages
(use-package delight)
...
*** Corgi packages broken up
#+begin_src emacs-lisp :noweb-ref corgi-setup
...
#+begin_src emacs-lisp :noweb-ref corgi-editor
...
#+begin_src emacs-lisp :noweb-ref corgi-bindings
...
#+begin_src emacs-lisp :noweb-ref corgi-keys
...
#+end_src
*** Tangle Emacs Startup enhancements and GC
:PROPERTIES:
...
#+begin_src emacs-lisp :noweb-ref editor-sanity :tangle no
(use-package emacs
...
#+begin_src emacs-lisp :noweb-ref abbrev
  (use-package abbrev
    ...
    :delight
    :config
    (setq abbrev-file-name (save-in-etc-file! "abbrev.el"))
...
#+begin_src emacs-lisp :noweb-ref appearance
  (use-package gas-ui
    ...
    (defgroup gas-ui ()
      ...
      :prefix "gas-ui-"
      :group 'emacs)
    (defcustom gas-ui-title-show-bufname t
      "Whether to include bufname in the title bar.
    Bufname is not necessary on GNOME but may be useful in other DEs."
      :type 'boolean
      :tag "Title bufname"
      :group 'gas-ui)
    (defcustom gas-ui-dark-theme 'modus-vivendi
      "Dark theme to use."
      :tag "Dark theme"
      :type 'symbol
      :group 'gas-ui)
    (defcustom gas-ui-light-theme 'modus-operandi
      "Light theme to use."
      :tag "Light theme"
      :type 'symbol
      :group 'gas-ui)
    (defvar gas-ui-line-pixel-height (line-pixel-height)
...
;;(use-package minions
;;  :hook global-corgi-stateline-mode)
...
(use-package eldoc
  :hook (elisp-mode clojure-mode cider-repl-mode lisp-interactive-mode)
  :config
...
(cider-register-cljs-repl-type 'nbb-repl "(+ 40 2)") ;;"(do (require '[promesa.core :as p]))")
...
(defun corgi/cider-jack-in-nbb (&optional project-dir)
  "Start a utility CIDER REPL backed by node Babashka, not related to a
specific project."
  (interactive)
  (let ((project-dir (or project-dir user-emacs-directory)))
    (nrepl-start-server-process
...
** Default coding setups - Markdown , YAML, Terrafom etc
